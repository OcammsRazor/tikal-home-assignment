...
const characterNamesToQueryParam = (characherNames: string[]) =>
  characherNames.join("&name=");
...
export const rickAndMortyClient = (baseUrl = DEFAULT_URL) => {
  ...
  return {
    ...
    getMultipleCharactersByNames: (characterNames: string[]) =>
      client.get(
        `/character?name=${characterNamesToQueryParam(characterNames)}`
      ),
  };
};
